...
# Группировка данных
salary_by_year = vacancies.groupby('year')['salary_rub'].mean().reset_index()
salary_by_year.rename(columns={'salary_rub': 'average_salary_rub'}, inplace=True)

# Округление средней зарплаты до двух знаков после запятой
salary_by_year['average_salary_rub'] = salary_by_year['average_salary_rub'].round(2)

print("четвертая часть готова")

# Сохранение таблицы в HTML с округленными значениями
salary_by_year.to_html('student_works/salary_by_year.html', index=False)

print("пятая часть готова")

# Построение графика с шагом в 1 год на оси X
plt.figure(figsize=(24, 10))
plt.plot(salary_by_year['year'], salary_by_year['average_salary_rub'], marker='o', color='#F4A460')
...
plt.xticks(salary_by_year['year'])

plt.savefig('student_works/salary_trends.png')
plt.show()
