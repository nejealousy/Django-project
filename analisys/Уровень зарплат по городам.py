...
salary_by_city = vacancies.groupby('area_name')['salary_rub'].mean().reset_index()

# Считаем долю вакансий по каждому городу
vacancies_count_by_city = vacancies['area_name'].value_counts(normalize=True) * 100  # Доля в процентах
...
# Округление значений средней зарплаты до двух знаков после запятой
salary_by_city_sorted['salary_rub'] = salary_by_city_sorted['salary_rub'].round(2)

# Сохранение таблицы в HTML
salary_by_city_sorted.to_html('student_works/salary_by_city_filtered.html', index=False)

# Построение графика для уровня зарплат по городам
plt.figure(figsize=(24, 10))
plt.barh(salary_by_city_sorted['area_name'], salary_by_city_sorted['salary_rub'], color='steelblue')
plt.title('Уровень зарплат по городам (с долей вакансий > 1%)', fontsize=14)
plt.xlabel('Средняя зарплата (руб.)', fontsize=12)
plt.ylabel('Город', fontsize=12)
plt.grid(True)

# Сохраняем график
plt.savefig('student_works/salary_by_city_filtered_trends.png')
plt.show()

print("График и таблица уровня зарплат по городам с долей вакансий > 1% сохранены в папку student_works")
